...
/**
 * <p>A pretty-printer to English. This is a wild experiment that repeats my attempts to explain TLA+ during
 * a code-reading session.</p>
 *
 * @author Igor Konnov
 */
class ProseWriter(writer: PrintWriter, layout: TextLayout = new TextLayout,
  ...
  def write(decl: TlaDecl): Unit = {
    prettyWriteDoc(toDoc(decl, isTopLevel = true) <> line <> line)
  }
  ...
  private def moduleNameDoc(name: String): Doc = {
    text("# Specification ") <> name <> line
  }

  private def moduleExtendsDoc(moduleNames: List[String]): Doc =
    if (moduleNames.isEmpty) emptyDoc
    else line <> "```" <> "Extends" <> space <> hsep(moduleNames.map(text), comma) <> "```" <> line
  ...
  def toDoc(mod: TlaModule, extensionModuleNames: List[String] = List.empty): Doc = {
    moduleNameDoc(mod.name) <>
      ...
      lsep((mod.declarations.toList.map(d => toDoc(d, isTopLevel = true))) :+ moduleTerminalDoc, line)
  }
  ...
  def toDoc(parentPrecedence: (Int, Int), expr: TlaEx): Doc = {
    expr match {
      ...
      case OperEx(op @ TlaSetOper.enumSet, arg) =>
        // a singleton set
        group(braces("Set" <> softline <> "of" <> softline <> toDoc(op.precedence, arg)))
      ...
      case OperEx(op @ TlaSetOper.enumSet, args @ _*) =>
        ...
        group(braces("Set" <> softline <> "of" <> softline <> group(nest(commaSeparated, layout.indent))))

      case OperEx(op @ TlaFunOper.tuple, arg) =>
        // a singleton tuple, e.g., (1,)
        group(text("(") <> toDoc(op.precedence, arg) <> comma <> ")")
      ...
      case OperEx(op @ TlaSetOper.times, args @ _*) =>
        // a cross product, e.g, A \X B \X C
        val argDocs = args.map(toDoc(op.precedence, _))
        val commaSeparated = ssep(argDocs.toList, text(",") <> softline)
        braces(group("Cross product of" <> softline <> nest(linebreak <> commaSeparated, layout.indent)))
      ...
      case OperEx(op, args @ _*) if op == TlaBoolOper.and || op == TlaBoolOper.or =>
        ...
        val sign = if (op == TlaBoolOper.and) "and" else "or"
      ...
      case OperEx(op @ TlaBoolOper.exists, x, set, pred) =>
        val doc =
          group(
              "Exists" <> space <> text(x.toString) <> space <>
                "in" <> softline <>
                toDoc(op.precedence, set) <> space <> text("such that")
      ...
      case OperEx(op @ TlaBoolOper.forall, x, set, pred) =>
        val doc =
          group(
              "For each" <> space <> text(x.toString) <> space <>
                "in" <> softline <>
                toDoc(op.precedence, set) <> space <> "holds" <>
                nest(line <> toDoc(op.precedence, pred)),
          )

        wrapWithParen(parentPrecedence, op.precedence, doc)

      case OperEx(op, x, set, pred) if op == TlaOper.chooseBounded =>
        val sign = ProseWriter.bindingOps(op)
        val doc =
          group(
              group(
                  text(sign) <> space <> text(x.toString) <> space <>
                    text(ProseWriter.binaryOps(TlaSetOper.in)) <> softline <>
      ...
      case OperEx(TlaFunOper.enum, keysAndValues @ _*) =>
        ...
        group(braces(nest(ssep(boxes.toList, comma <> line))))
      ...
      case OperEx(TlaSetOper.recSet, keysAndValues @ _*) =>
        ...
        // format each key-value pair (k, v) into k: v
        val boxes =
          keys.zip(values).map(p => group(strNoQuotes(p._1) <> space <> "in" <> nest(line <> toDoc((0, 0), p._2)))) ///

        group(
            "Set" <> space <> "of" <> space <> "records" <> space <>
              nest(brackets(ssep(boxes.toList, comma <> line))))
      ...
      case OperEx(TlaFunOper.funDef, body, keysAndValues @ _*) =>
        ...
        group(
            "Function" <> space <> "of" <>
              nest(line <> brackets(binders) <> space <> "to" <> nest(line <> bodyDoc)),
        ) ////
      ...
      case OperEx(TlaSetOper.map, body, keysAndValues @ _*) =>
        ...
        val bodyDoc = toDoc((0, 0), body)
        group(
            braces("Set" <> space <> "of" <> nest(line <> bodyDoc)) <> space <> "where" <> nest(
                line <> braces(binders)))
      ...
      case OperEx(TlaSetOper.filter, name, set, pred) =>
        ...
        group(
            braces("Set" <> space <> "of" <> space <> binding) <> nest(line <> "such" <> space <> "that") <>
              nest(line <> filter) <>
              nest(line <> "holds"),
        ) ///
      ...
      // seq1 \o seq2
      case OperEx(TlaSeqOper.concat, seq1, seq2) =>
        group(
            "Concat" <> space <> toDoc(TlaSeqOper.concat.precedence, seq1) <> space <> "with" <> space <> toDoc(
                TlaSeqOper.concat.precedence, seq2) <> ".",
        ) ///

      // Append(seq1, seq2)
      case OperEx(TlaSeqOper.append, seq1, seq2) =>
        group(
            "Append" <> space <> toDoc(TlaSeqOper.concat.precedence, seq1) <> space <> "to" <> space <> toDoc(
                TlaSeqOper.concat.precedence, seq2) <> ".",
        ) ///
      ...
      case OperEx(TlaFunOper.except, funEx, keysAndValues @ _*) =>
        ...
        val boxes =
          indexDocs
            ...
            .map { case (index, value) =>
              group("at" <> space <> index <> space <> "set" <>
                    nest(line <> value))
            } ///
        ...
        val doc =
          "Copy" <> space <> "function" <>
            nest(line <> toDoc(TlaFunOper.except.precedence, funEx) <>
      ...
      case OperEx(TlaSetOper.funSet, domain, coDomain) =>
        val domDoc = toDoc(TlaSetOper.funSet.precedence, domain)
        val cdmDoc = toDoc(TlaSetOper.funSet.precedence, coDomain)
        "Set" <> space <> "of" <> space <> "functions" <> nest(line <> "from" <> space <> domDoc) <>
          nest(line <> "to" <> space <> cdmDoc)
      ...
      case OperEx(op @ TlaBoolOper.not, arg) =>
        // Boolean negation
        val doc = "not" <> parens(toDoc((0, 0), arg))
        wrapWithParen(parentPrecedence, op.precedence, doc)

      case OperEx(op, arg @ NameEx(_)) if ProseWriter.unaryOps.contains(op) =>
        val doc = text(ProseWriter.unaryOps(op)) <> toDoc(op.precedence, arg)
        wrapWithParen(parentPrecedence, op.precedence, doc)

      case OperEx(op, arg @ ValEx(_)) if ProseWriter.unaryOps.contains(op) =>
        val doc = text(ProseWriter.unaryOps(op)) <> toDoc(op.precedence, arg)
        wrapWithParen(parentPrecedence, op.precedence, doc)

      case OperEx(op, arg @ OperEx(_, _)) if ProseWriter.unaryOps.contains(op) =>
      ...
      case OperEx(op, arg) if ProseWriter.unaryOps.contains(op) =>
        // in all other cases, introduce parentheses.
        // Yse the minimal precedence, as we are introducing the parentheses in any case.
        text(ProseWriter.unaryOps(op)) <> parens(toDoc((0, 0), arg))

      // a binary infix operator that is mapped to Apalache IR
      case OperEx(op, lhs, rhs) if ProseWriter.binaryOps.contains(op) =>
        val doc =
          toDoc(op.precedence, lhs) <>
            nest(
                line <>
                  text(ProseWriter.binaryOps(op)) <> space <>
                  toDoc(op.precedence, rhs))
        wrapWithParen(parentPrecedence, op.precedence, group(doc))

      // a user-defined binary infix operator such as :> or @@
      case OperEx(TlaOper.apply, NameEx(operName), lhs, rhs) if ProseWriter.userDefinedBinaryOps.contains(operName) =>
        val doc =
          toDoc((0, 0), lhs) <>
            nest(
                line <>
      ...
      case OperEx(op, args @ _*) if ProseWriter.naryOps.contains(op) =>
        val sign = ProseWriter.naryOps(op)
        val argDocs = args.map(toDoc(op.precedence, _)).toList
        val doc = nest(folddoc(argDocs, _ <> line <> sign <> space <> _))
        wrapWithParen(parentPrecedence, op.precedence, group(doc))
      ...
      case LetInEx(body, decls @ _*) =>
        def eachDecl(d: TlaOperDecl) = {
          group(toDoc(d, isTopLevel = false) <> line <> "in")
        }
  ...
  def toDoc(decl: TlaDecl, isTopLevel: Boolean): Doc = {
    val annotations = declAnnotator(layout)(decl)

    decl match {
      case TlaConstDecl(name) =>
        val doc = line <> "```" <> line <> "Given" <> space <> "constant" <> space <> name <> line <> "```"
        if (annotations.isEmpty) {
          doc
        } else {
          fromAnnotations(annotations.get) <> line <> doc
        }

      case TlaVarDecl(name) =>
        val doc = line <> "```" <> line <> "Introduce" <> space <>
          "state" <> space <> "variable" <> space <> name <> line <> "```"
        if (annotations.isEmpty) {
          doc
        } else {
          fromAnnotations(annotations.get) <> line <> doc
      ...
      case TlaAssumeDecl(body) =>
        val doc = line <> "```" <> line <> "Assume" <> space <>
          "that" <> space <> toDoc((0, 0), body) <> line <> "```"
        if (annotations.isEmpty) {
          doc
        } else {
          fromAnnotations(annotations.get) <> line <> doc
      ...
      case TlaOperDecl(name, List(), OperEx(TlaFunOper.recFunDef, body, keysAndValues @ _*)) =>
        ...
        val binding = binders
        // f[x \in S] == e
        val defOrLet = if (isTopLevel) "Define" else "Let"
        val asOrBe = if (isTopLevel) "as" else "be"
        val doc =
          defOrLet <> space <> "recursive" <> space <> "function" <>
            nest(line <> group(parseableName(name) <> brackets(binding)) <> space <> asOrBe) <>
            nest(line <> toDoc((0, 0), body))
        recFunName = REC_FUN_UNDEFINED
        if (annotations.isEmpty) {
          group(doc)
        } else {
          wrapWithComment(fromAnnotations(annotations.get)) <> line <> group(doc)
        }
      ...
      case tod @ TlaOperDecl(name, params, body) =>
        ...
        val defOrLet = if (isTopLevel) "Define" else "Let"
        val asOrBe = if (isTopLevel) "as" else "be"
        val recursive: Doc = if (tod.isRecursive) "recursive" <> space else ""
        val declDoc =
          group(
              defOrLet <> space <> recursive <> parseableName(name) <> paramsDoc <> space <> asOrBe <>
                nest(line <> toDoc((0, 0), body)))
        val quotedDeclDoc =
          if (!isTopLevel) declDoc else line <> "```" <> line <> declDoc <> line <> "```"
        if (annotations.isEmpty) {
          quotedDeclDoc
        } else {
          fromAnnotations(annotations.get) <> line <> quotedDeclDoc
        }
    }
  }
  ...
  private def fromAnnotations(annotations: List[String]): Doc = {
    lsep(annotations.map(text), line)
  }
...
object ProseWriter {
  ...
  protected val binaryOps =
    HashMap(
        ...
        TlaBoolOper.equiv -> "if and only if",
        TlaArithOper.plus -> "+",
        TlaArithOper.minus -> "-",
        TlaArithOper.mult -> "*",
        TlaArithOper.div -> "/",
        ...
        TlaArithOper.ge -> ">=",
        TlaSetOper.in -> "in",
        TlaSetOper.notin -> "not in",
        TlaSetOper.cap -> "intersection",
        TlaSetOper.cup -> "union",
        TlaSetOper.setminus -> "\\",
        TlaSetOper.subseteq -> "is subset of",
        TlaActionOper.composition -> "then",
        TlaTempOper.leadsTo -> "~>",
